...
app.get('/articles/:article_name',function(req,res){
    ...
    pool.query("select a.title,a.content,c.comments from article a left join articlecomments c on a.Title= c.articletitle where a.name=$1",[req.params.article_name],function(err,results){
        if(err)
            res.status(500).send(err.toString());
        else
